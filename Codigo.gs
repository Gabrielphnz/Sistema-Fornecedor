...
// FIX 3: Gerar PDF formatado corretamente e convertido para binário PDF
function gerarPDFPedido(idPedido, token) {
  ...
  // A CORREÇÃO: getAs('application/pdf') obriga o servidor a renderizar o HTML num PDF real
...
function getDashboardData(token) {
  ...
  pedidos.forEach(p => {
    ...
    if (!pedidosPorFornecedor[fornecedorNome]) pedidosPorFornecedor[fornecedorNome] = { total: 0, entregues: 0 };
    pedidosPorFornecedor[fornecedorNome].total++;
    if (p[3] === "Entregue") pedidosPorFornecedor[fornecedorNome].entregues++;
  });
  return { pedidosPorFornecedor, ranking: [] }; // simplificado para caber
}
